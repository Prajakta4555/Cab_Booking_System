          ...
          <?php
            ...
            if ($result->num_rows > 0) {
              while($row = $result->fetch_assoc()) {
                $count++;
                  echo  "<tr><td> ". $count."</td><td> ". $row["cab_id"]. "</td><td> " . $row["model"] ."</td><td> ".$row["cab_year"]."</td><td> ".$row["cab_type"]."</td><td> ".$row["status"]."</td><td>".$row["rent"]."</td><td>".$row["driver_id"]."</td><td>".$row["owner_id"]."</td></tr> ";
              }  
           } 
              if($count==0)
              {
